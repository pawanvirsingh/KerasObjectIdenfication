...
class EditObject(LoginRequiredMixin, generic.TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if "profile_form" not in kwargs:
            kwargs["profile_form"] = forms.ObjectForm(instance=user.profile)
        return super(EditProfile, self).get(request, *args, **kwargs)

    def post(self, request, *args, **kwargs):
        user = self.request.user
        user_form = forms.UserForm(request.POST, instance=user)
        profile_form = forms.ObjectForm(request.POST,
                                         request.FILES,
        ...
        if not (user_form.is_valid() and profile_form.is_valid()):
            ...
            profile_form = forms.ObjectForm(instance=user.profile)
